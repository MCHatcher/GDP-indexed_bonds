//Dynare code for simulating the extended model (Figs 5-6 in the paper). 
...
figure(1)
subplot(1,2,1), plot(v_stack,Stack_Lambda,'k','LineWidth', 1),   //hold on, plot(v_stack,Stack_Lambda1,'--k','LineWidth', 1)
title('Welfare'), hold on, xlabel('Indexation parameter ($v$)'), ylabel('% cons. equiv.'), hold on,
subplot(1,2,2), plot(v_stack,Lambda_mean,'k','LineWidth', 1), hold on, plot(v_stack,Lambda_var,'--k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), ylabel('% cons. equiv.'), title('Welfare decomposition')
yline(0,'--k')
//subplot(2,2,3), plot(v_stack,Stack_tau/Stack_tau(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), ylabel('Ratio'), title('Mean taxes: $E[\tau_t]$')
//subplot(2,2,4), plot(v_stack,var_tau/var_tau(1),'k','LineWidth', 1), ylabel('Ratio'), xlabel('Indexation parameter ($v$)'), title('Variance of taxes: $var[\tau_t]$')

figure(2)
subplot(2,4,1), plot(v_stack, Stack_c1/Stack_c1(1),'k','LineWidth', 1), hold on, plot(v_stack, Stack_c2/Stack_c2(1),'--k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Consumption means: $E[c_{j,t}]$')
subplot(2,4,2), plot(v_stack, var_c1/var_c1(1),'k','LineWidth', 1), hold on, plot(v_stack, var_c2/var_c2(1),'--k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Consumption variance: $var[c_{j,t}]$')
subplot(2,4,3), plot(v_stack, Stack_tau/Stack_tau(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Mean taxes: $E[\tau_t]$')
subplot(2,4,4), plot(v_stack, var_tau/var_tau(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Variance of taxes: $var[\tau_t]$')
subplot(2,4,5), plot(v_stack, Stack_ri/Stack_ri(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Mean return: $E[r^i_t]$')
subplot(2,4,6), plot(v_stack, var_ri/var_ri(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Return risk: $var[r^i_t]$')
subplot(2,4,7), plot(v_stack, Stack_k/Stack_k(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Mean capital: $E[k_t]$')
subplot(2,4,8), plot(v_stack, Stack_y/Stack_y(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Mean output: $E[y_t]$')