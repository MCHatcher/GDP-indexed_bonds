...
figure(1)
subplot(2,2,1), plot(v_stack,Lambda_tau,'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), ylabel('% tax equiv.'), title('Tax burden vs indexation')
yline(0,'--k')
...
figure(2)
subplot(2,3,1), plot(v_stack, Stack_c1/Stack_c1(1),'k','LineWidth', 1), hold on, plot(v_stack, Stack_c2/Stack_c2(1),'--k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Consumption means: $E[c_{j,t}]$')
subplot(2,3,2), plot(v_stack, var_c1/var_c1(1),'k','LineWidth', 1), hold on, plot(v_stack, var_c2/var_c2(1),'--k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Consumption variance: $var[c_{j,t}]$')
subplot(2,3,3), plot(v_stack, Stack_tau/Stack_tau(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Mean taxes: $E[\tau_t]$')
subplot(2,3,4), plot(v_stack, var_tau/var_tau(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Variance of taxes: $var[\tau_t]$')
subplot(2,3,5), plot(v_stack, Stack_ri/Stack_ri(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Mean return: $E[r^i_t]$')
subplot(2,3,6), plot(v_stack, var_ri/var_ri(1),'k','LineWidth', 1), xlabel('Indexation parameter ($v$)'), title('Return risk: $var[r^i_t]$')

figure(3)
subplot(1,2,1), hold on, plot(v_stack,Lambda_tau,'--g','LineWidth', 1), xlabel('Indexation parameter ($v$)'), ylabel('% tax equiv.'), title('Tax burden vs indexation')
//yline(0,'--k'), //legend('Line 1','Line 2','Line 3','') 
subplot(1,2,2), hold on, plot(v_stack,Lambda_v,'--g','LineWidth', 1), xlabel('Indexation parameter ($v$)'), ylabel('% c.e. welfare gain'), title('Welfare gain of indexation')
//yline(0,'--k')
