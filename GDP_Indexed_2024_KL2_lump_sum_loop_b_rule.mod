//Dynare code for simulating the extended model: bond rule sensitivity analysis. 
...
parameters alfa, betta, chi, chi1, v, deltta, eps, gama, tau_p, thetta, n, psi, gbar, bstar, c1star, c2star, utilitystar, taustar, rstar, 
sig_A, sig_b; 

alfa = 0.3;  
betta = 0.9; 
deltta = 0.5;  //Rule 1  1/3,2/3,-1/3,-2/3 Rules 2 and 3 -0.15, -0.3 
gama = 5;  
eps = 0.5;  
n = 0.4;  
chi = 1+n;
...
figure(1)
//hold on, subplot(1,3,3), plot(v_stack,Stack_Lambda,'k','LineWidth', 1),   //hold on, plot(v_stack,Stack_Lambda1,'--k','LineWidth', 1),
hold on, plot(v_stack,Stack_Lambda,'--k','LineWidth', 1)
hold on, xlabel('Indexation parameter ($v$)'), ylabel('% cons. equiv.'), hold on,
//title('Rule 1: $b_t = \overline{b} + \delta(b_{t-1} - \overline{b})+\varepsilon_{b,t}$')
//title('Rule 2: $b_t = \overline{b} + \delta(y_{t} - y_{SS})$')
title('Rule 3: $b_t = \overline{b} + \delta(\tau_t y_{t} - \tau_{SS}y_{SS})$')
